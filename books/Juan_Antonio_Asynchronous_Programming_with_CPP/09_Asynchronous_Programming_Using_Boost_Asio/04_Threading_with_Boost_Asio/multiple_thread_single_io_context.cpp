...
References
    ...
    Code https://godbolt.org/z/E3cjod6z9
...
void executeit(boost::asio::io_context & l_io_context, const int i) {

    boost::asio::post(l_io_context,

    /*
        [i=i](const boost::system::error_code& l_err){
            
            if(l_err) {
                std::cout << "Error message " << l_err.message() << '\n';
            } else {
...
int main() {
    ...
    for(int i = 0; i < 5; ++i) {
        threads.emplace_back(executeit, std::ref(l_io_context), i);
    }
